input_text = '''LLLLL.LLLL.LLLLLLLLL.LLLLLL.LLLLLLL.LLLLLL.LLLLLLLLLL.LLLLLLLL.LLLLLL.LLLLLLLLLLL.LLLLLLLLLLLLLLLL
...
import copy
seat_map = [[c for c in row] for row in input_text.split('\n')]

width = len(seat_map[0])
height = len(seat_map)


def run(step_limit=None, occupied_tolerance=5):
    def count_adj(row, col, current_map, check_symbol):
        count = 0
        ...
        for direction in [(-1, -1), (-1, 0), (-1, 1),
            ...
            steps = 0
            while not (step_limit and steps >= step_limit):
                test_row, test_col = test_row+direction[0], test_col+direction[1]
                if 0 <= test_row < height and 0 <= test_col < width:
                    if current_map[test_row][test_col] == check_symbol:
                        count += 1
                    if current_map[test_row][test_col] in ('L', '#'):
                        # Stop checking that direction if Any seat is found
                        break
                else:
                    # Stop checking that direction if edge is hit
                    break
                steps += 1
        return count
    ...
    while map_changed:
        ...
        for i in range(height):
            for j in range(width):
                adj_occupied_seats = count_adj(i, j, current_map, check_symbol='#')
                if current_map[i][j] == '#' and adj_occupied_seats >= occupied_tolerance:
                    new_map[i][j] = 'L'
                    map_changed = True
                elif current_map[i][j] == 'L' and adj_occupied_seats == 0:
                    new_map[i][j] = '#'
                    map_changed = True
        current_map = new_map

    # Num of occupied seat at the end
    return sum([row.count('#') for row in current_map])


# Part 1
print(run(step_limit=1, occupied_tolerance=4))

# Part 2
print(run(step_limit=None, occupied_tolerance=5))