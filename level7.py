input_text = '''dim red bags contain 2 bright gold bags, 5 striped fuchsia bags.
...
# process rules
part1_rules = defaultdict(lambda:defaultdict(int))  # color: {carriable_by_color: num}
part2_rules = defaultdict(lambda:defaultdict(int))  # color: {carry_color: num}
for row in input_text.split('.\n'):
    color, contain_rule = row.split(' bags contain ')
    for bag_text in contain_rule.split(', '):
        tokens = bag_text.split(' ')
        contain_color = ' '.join(tokens[1:-1])
        if tokens[0].isnumeric():
            num = int(tokens[0])
            part1_rules[contain_color][color] = num
            part2_rules[color][contain_color] = num
# import pprint
# pprint.pprint(part2_rules)

def find_outermost_bags(color):
    import queue
    q = queue.Queue()
    q.put( (color, 1))
    ...
    while not q.empty():
        color, num = q.get()
        for carry_by_color, carry_by_color_num in part1_rules[color].items():
            results[carry_by_color] += carry_by_color_num
            q.put( (carry_by_color, carry_by_color_num*num) )
    return results

# part 1
print(len(find_outermost_bags('shiny gold')))
def find_contained_bags(color):
    ...
    while not q.empty():
        color, num = q.get()
        for carry_color, carry_color_num in part2_rules[color].items():
            results[carry_color] += carry_color_num*num
            q.put((carry_color, carry_color_num * num))
    return results

results = find_contained_bags('shiny gold')
print(sum([num for color, num in results.items()]))