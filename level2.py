...
for row in input_text.split('\n'):
    condition, password = row.split(': ')
    range, required_char = condition.split(' ')
    n1, n2 = range.split('-')
    n1, n2 = int(n1), int(n2)

    # Part 1
    # occurence = password.count(required_char)
    # if occurence >= n1 and occurence <= n2:
    #     valid_password_count = valid_password_count + 1

    # Part 2
    if (password[n1-1] == required_char) != (password[n2-1] == required_char):
