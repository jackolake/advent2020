input_text = '''eyr:2021 hgt:168cm hcl:#fffffd pid:180778832 byr:1923 ecl:amb iyr:2019 cid:241
...
def is_valid(field_name, field_value):
    hcl_p = re.compile('^([0-9]|[a-z]){6}$')
    pid_p = re.compile('[0-9]{9}')
    if field_name == 'byr':
        if len(field_value) == 4 and int(field_value) >= 1920 and int(field_value) <= 2002:
            return True
    elif field_name == 'iyr' and len(field_value) == 4 and int(field_value) >= 2010 and int(field_value) <= 2020:
        return True
    elif field_name == 'eyr' and len(field_value) == 4 and int(field_value) >= 2020 and int(field_value) <= 2030:
        return True
    elif field_name == 'hgt':
        if field_value.endswith('cm') or field_value.endswith('in'):
            v = int(field_value[:-2])
            if (field_value.endswith('cm') and v >= 150 and v<= 193) \
                    or (field_value.endswith('in') and v >= 59 and v <= 76):
                return True
    elif field_name == 'hcl' and field_value[0] == '#' and hcl_p.match(field_value[1:]):
        return True
    elif field_name == 'ecl' and field_value in ('amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'):
        return True
    elif field_name == 'pid' and len(field_value) == 9 and pid_p.match(field_value):
        return True
    return False

for passport_data in input_text.split('\n\n'):
    missing_fields = set(required_fields)
    invalid_fields = set(required_fields)
    for row in passport_data.split('\n'):
        for field in row.split(' '):
            field_name, field_value = field.split(':')
            missing_fields = missing_fields - set([field_name])
            if is_valid(field_name, field_value):
                invalid_fields = invalid_fields - set([field_name])
